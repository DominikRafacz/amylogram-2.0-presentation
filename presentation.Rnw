...
\begin{frame}{Results}
<<res,warning=FALSE,message=FALSE,echo=FALSE,include=TRUE,fig.height=4.5,results = "hide">>=
results <- readr::read_csv("results/all_models_scores.csv")

library(tidyverse)
results %>%
  select(-pos, -nice_name, -X1) %>%
  ...
  mutate(classifier = fct_reorder(fct_recode(as.factor(classifier), 
                              `AmyloGram 2.0 full` = "full_alphabet",
                              `AmyloGram 2.0 best` = "best_alphabet",
                              `AmyloGram full` = "raw_aa_10",
                              `AmyloGram best` = "class14596_10"),
                              1:7)) %>%
